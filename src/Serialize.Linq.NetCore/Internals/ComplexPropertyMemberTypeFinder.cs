    ...
    internal class ComplexPropertyMemberTypeFinder
    {
        ...
        private bool AnalyseType(Type baseType, ISet<Type> seen, ISet<Type> result)
        {
            ...
            if (baseType.GetTypeInfo().IsGenericType)
                retval = this.AnalyseTypes(baseType.GetTypeInfo().GetGenericArguments(), seen, result) || retval;
            retval = this.AnalyseTypes(baseType.GetTypeInfo().GetInterfaces(), seen, result) || retval;
